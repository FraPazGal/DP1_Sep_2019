...
public class ConferenceService {
	...
	public Conference reconstruct(final Conference conference,
		...
		if (conference.getId() != 0) {
			...
			Assert.isTrue(aux.getAdministrator().equals((Administrator) principal));
		...
		try {
			Assert.isTrue(conference.getSubmissionDeadline().before(conference.getNotificationDeadline()),"submission.before.notification");
		} catch (final Exception e) {
			binding.rejectValue("submissionDeadline", "submission.before.notification");
		}
		
		try {
			Assert.isTrue(conference.getNotificationDeadline().before(conference.getCameraReadyDeadline()),"notification.before.camera");
		} catch (final Exception e) {
			binding.rejectValue("notificationDealine", "notification.before.camera");
		}
		
		try {
			Assert.isTrue(conference.getCameraReadyDeadline().before(conference.getStartDate()),"camera.before.start");
		} catch (final Exception e) {
			binding.rejectValue("cameraReadyDeadline", "camera.before.start");
		}
		
		try {
			Assert.isTrue(conference.getStartDate().before(conference.getEndDate()),"start.before.end");
		} catch (final Exception e) {
		...
		try {
			...
			Assert.isTrue(conference.getSubmissionDeadline().after(now),"submission.after.now");
		} catch (final Exception e) {
