...
public interface FinderRepository extends JpaRepository<Finder, Integer> {
	...
	@Query("select distinct c from Conference c where ( c.title like %?1%% or c.acronym like %?1%% or c.summary like %?1%% or c.venue like %?1%%) and  c.entryFee <= ?2 and c.startDate >= ?3 and c.endDate <= ?4 and c.category.id = ?5 and c.isFinal = true")
	Collection<Conference> searchWithCat(String keyWord, Double maximumFee,
			Date minimumDate, Date maximumDate, Integer categoryId);
